...
ax1.plot(df["px_true"], df["py_true"], df["pz_true"], 'k-', label="Vrai", linewidth=2)
...
ax2.plot(df["t"], err, 'b-')
ax2.set_title("Erreur position (m)")
ax2.set_xlabel("t (s)")
ax2.grid(True)

ax3 = fig.add_subplot(133)
ax3.plot(df["px_true"], df["py_true"], 'k-', label="Vrai", linewidth=2)
ax3.plot(df["px_est"],  df["py_est"],  'b-', label="EKF",  linewidth=1.5)
ax3.scatter(df_gps["px_gps"], df_gps["py_gps"], c='r', s=5, label="GPS")
ax3.set_title("Vue de dessus (x-y)")
...
plt.show()
print("Graphe sauvegardé dans results.png")
